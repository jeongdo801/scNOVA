...
for (i in 1:nrow(sv_calls_all)){
	...
	if (sum(subclone[,1]==sv_calls_all[i,5])==0){sv_calls_all[i, ncol(sv_calls_all)] <- "clone0"}
}

for (k in 1:length(unique(subclone[,2]))){
	subclone_lab <- paste0("clone", k)
	filename1 <- paste0("input_user/CN_matrix_for_Genes_", subclone_lab, ".txt")
	filename2 <- paste0("input_user/CN_matrix_for_DHS_", subclone_lab, ".txt")
	...
	for (i in 1:nrow(TSS_matrix)){
  	...
  	##1) CN for TSS
  	CN_TSS <- rbind(tmp[tmp[,2]>TSS_matrix[i,3] & tmp[,3]<TSS_matrix[i,4],],
  	...
  	##2) CN for Genebody
  	CN_GB <- rbind(tmp[tmp[,2]>Genebody_matrix[i,2] & tmp[,3]<Genebody_matrix[i,3],],
  	...
  	##3) CN for TES
  	CN_TES <- rbind(tmp[tmp[,2]>TES_matrix[i,3] & tmp[,3]<TES_matrix[i,4],],
	...
	for (i in 1:nrow(DHS_matrix)){
  	...
  	CN_DHS <- rbind(tmp[tmp[,2]>DHS_matrix[i,3] & tmp[,3]<DHS_matrix[i,4],],
