...
Expressed_pred <- read.table("result_CNN/Expressed_train80_final_result.txt", sep = '\t', header=T, comment.char = "")
...
source("utils/script_PLSDA/auto_R.R")
...
for (aj in 2:lv){
  for (ai in 1:nrow(X_auto)){
    ...
    source("utils/script_PLSDA/Pred_PLS_R_scNOVA.R")
...
source("utils/script_PLSDA/vip_R_v2_scNOVA.R")
result_vip_all = vip_R_v2(w, r2)


#Generation of null distribution of VIP
source("utils/script_PLSDA/vip_null_R.R")
perm = 100
